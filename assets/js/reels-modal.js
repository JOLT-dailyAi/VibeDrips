...
const REELS_DATA = {
  reels: [
    ...
    // Add more reels here for testing
  ]
};
...
function loadReel(reelData) {
  ...
  // ========================================
  // CHANGE THIS VALUE TO TEST DIFFERENT METHODS:
  // 'embed' = Instagram official embed
  // 'link' = Button that opens Instagram
  // 'iframe' = Direct iframe (will likely fail)
  // ========================================
  const TEST_METHOD = 'embed';  // ← CHANGE THIS TO: 'embed', 'link', or 'iframe'
  
  
  if (TEST_METHOD === 'embed') {
  ...
  else if (TEST_METHOD === 'iframe') {
    // Method 3: Direct iframe (likely blocked)
    console.log('Testing: Direct iframe (may be blocked)');
    try {
      ...
      embedContainer.innerHTML = `
        <div style="text-align: center; padding: 40px; color: red;">
          <p>Error: Could not load Instagram reel via iframe</p>
        </div>
      `;
    }
  }
